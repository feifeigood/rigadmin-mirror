        ...
        >
          ...
          <el-table-column label="名称" align="left" prop="job" :show-overflow-tooltip="true">
            <template slot-scope="scope">
              <span class="link-type" @click="handleBindRule(scope.row)">{{ scope.row.job }}</span>
            </template>
          </el-table-column>
...
import RuleSample from './components/rulesample.vue';

export default {
  name: "DefaultExporter",
  components: { RuleSample },
  data() {
    return {
      showRuleSample: false,
  ...
  methods: {
    ...
    handleBindRule(row){
      this.showRuleSample = true;
      this.selectedExporterId = row.id;
    }
