...
// alertpager 实例
const alertpager = axios.create({
  baseURL: process.env.VUE_APP_ALERTPAGER_API,
  timeout: 10000
})

// request拦截器
alertpager.interceptors.request.use(config => {
  // 是否需要设置 token
  const isToken = (config.headers || {}).isToken === false
  if (getToken() && !isToken) {
    config.headers['Authorization'] = 'Bearer ' + getToken() // 让每个请求携带自定义token 请根据实际情况自行修改
...
// 响应拦截器
alertpager.interceptors.response.use(res => {
  console.log(res);
  // 未设置状态码则默认成功状态
  const code = res.data.code || 200;
  // 获取错误信息
